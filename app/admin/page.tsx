...
export default function AdminPage() {
  const { config, setConfig, adminForm, setAdminForm, addRecord, showToast } = useAppStore();
  const { t } = useTranslation();

  // 管理员拨币
  const handleAdminDistribute = () => {
    if (!adminForm.amount || parseFloat(adminForm.amount) <= 0) {
      showToast(t('toast_valid_amount'));
      return;
    }
    addRecord(parseFloat(adminForm.amount), 'Admin');
    showToast(t('toast_admin_success'));
    setAdminForm({ targetAddr: '', amount: '' });
  };
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setAdminForm({ ...adminForm, targetAddr: e.target.value })}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              onChange={(e) => setConfig({ ...config, bonusRate: parseFloat(e.target.value) })}
          ...
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              onChange={(e) => setConfig({ ...config, lockDays: parseInt(e.target.value) })}
              className="w-20 bg-black/40 border border-white/10 rounded p-1 text-center font-tech text-white"
            />
          </div>
          <button
